...
- `src/resources` - one client per resource: `delegates`, `delegateQuestions` (ask/search/admin-review delegate Q&A - currently unmounted server-side, see `delegateQuestionsPath`), `voteResults`, `govProposals`, `decrees`, `reference` (parties/seats/topics/periods/plenary calendar), `account` (profile, bookmarks, topic selection, mail preferences, email changes, push notification tokens/settings), `events`, `statistics`.
